//! Shared frame conversion logic for serializing/deserializing Frames.
//!
//! Extracted from DiskPageStore so that both DiskPageStore and DiskFrameStore
...
/// Converts Frames to/from their serializable proxy types (SFrame, etc.).
///
/// Holds registries for fonts, images, and side tables for non-serializable
/// objects (tags, gradients, tilings). Shared between DiskPageStore and
/// DiskFrameStore.
...
impl FrameConverter {
    ...
    pub fn convert_frame(&mut self, frame: &Frame) -> SFrame {
    ...
    pub fn convert_frame_item(&mut self, item: &FrameItem) -> SFrameItem {
        match item {
    ...
    pub fn convert_group(&mut self, g: &GroupItem) -> SGroupItem {
        SGroupItem {
    ...
    pub fn convert_text(&mut self, t: &TextItem) -> STextItem {
    ...
    pub fn convert_fixed_stroke(&mut self, s: &FixedStroke) -> SFixedStroke {
        SFixedStroke {
    ...
    pub fn convert_paint(&mut self, paint: &Paint) -> SPaint {
        match paint {
    ...
    pub fn convert_destination(&self, dest: &Destination) -> SDestination {
        match dest {
    ...
    pub fn convert_tag(&mut self, tag: &Tag) -> STag {
        match tag {
    ...
    // --- Reconstruction: SFrame → Frame ---

    pub fn reconstruct_frame(&self, sf: SFrame) -> Frame {
    ...
    pub fn reconstruct_frame_item(&self, si: SFrameItem) -> FrameItem {
        match si {
    ...
    pub fn reconstruct_group(&self, sg: SGroupItem) -> GroupItem {
        GroupItem {
    ...
    pub fn reconstruct_fixed_stroke(&self, ss: SFixedStroke) -> FixedStroke {
        FixedStroke {
...
pub fn convert_color(c: &Color) -> SColor {
...
pub fn reconstruct_color(sc: SColor) -> Color {
    match sc {
